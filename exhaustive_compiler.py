...
def compile_exhaustive(episode, char_class, faction, source_path, output_path):
    ...
    character = CharacterStats(
        ...
        location=CAMPAIGN_EPISODES.get(episode, {}).get("location", "Târgoviște")
    )
    
    # Apply Class Stats (Simplified defaults)
    ...
    while queue and processed_count < MAX_STATES:
        ...
        for sug in current_suggestions:
            try:
                ...
                # We must COPY the state because we branch here
                # Pydantic models are mutable, deepcopy is needed or model_copy(deep=True)
